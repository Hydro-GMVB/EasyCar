...
# conn → Criação do arquivo "carros.db" na pasta "db"
# c → Permite a execução de comandos SQLITE no python
conn = sqlite3.connect("db/carros.db")
c = conn.cursor()

#! Criação da tabela padrão da aplicação
...
def jsonToDB():
    ...
    # a variável "data" irá "ler" o arquivo "info.json"
    with open("carros.json") as json_file:
